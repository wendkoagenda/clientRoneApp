...
import { notification, Table, DatePicker, Tooltip, Switch, Popconfirm, Checkbox, BackTop } from 'antd';
...
import { actions } from './report-list.reducer';
...
const ReportList = (props) => {
    ...
    const loadReportList = useCallback(async () => {
        try {
            const reportListRepose = await ReportsService.searchReportListByRequest(reportListSearchRequest);
    ...
    const viewReport = (record) => {
        if (record.reportTypeId == reportTypesIds.GENERIC_FIXTURE) {
            ...
            history.push({
                pathname: routes.GET_REPORT_ITEM_ROUTE(record.dispatchRequestWorkOrderId, record.reportTypeId)
            })
        }
    }
    ...
    const columns = [
        ...
        },
        {
            ...
            render: text => <div className="info-col"><p className="col-text">{moment(text).utcOffset(DEFAULT_UTC_OFFSET).format(strings.FIELD_FORMATS.DEFAULT_DATE_TIME_FORMAT)}</p></div>
        },
        {
            title: 'Report Number',
            dataIndex: 'id',
            key: 'id',
            render: text => <div className="info-col"><p className="col-text">{AddZerosToReportNumber(text)}</p></div>
        },
        {
            title: 'Status',
            ...
            render: text => <div className="info-col"><p className="col-text">{invoiceOrderStatusesWithNames.find(item => item.id == text).name}</p></div>,
        },
        {
            title: 'Section',
